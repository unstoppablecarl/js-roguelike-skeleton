...
var yuiDataTask = function(files, metalsmith, done){
    ...
    for(var key in files){
        ...
        if(file.docs_class){
            ...
            file.yui_class_url = makeUrl(file.docs_class);

            if(file.docs_method){
                file.yui_method_url = file.yui_class_url + '#method_' + file.docs_class;
            }
        }
        ...
        if(relatedDocs){
            ...
            for (var j = 0; j < relatedDocsFormatted.length; j++) {
                var relatedDoc = relatedDocsFormatted[j];

                var url = relatedDoc.url;
                if(url.indexOf('#') !== -1){
                    url = url.split('#')[0];
                }
            }
